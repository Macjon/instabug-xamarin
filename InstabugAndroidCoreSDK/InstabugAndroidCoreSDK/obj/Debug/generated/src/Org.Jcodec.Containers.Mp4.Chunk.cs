...
namespace Org.Jcodec.Containers.Mp4 {
	...
	public partial class Chunk : global::Java.Lang.Object {
		...
		public unsafe Chunk (long offset, long startTv, int sampleCount, int sampleSize, int[] sampleSizes, int sampleDur, int[] sampleDurs, int entry)
			: base (IntPtr.Zero, JniHandleOwnership.DoNotTransfer)
		{
			if (((global::Java.Lang.Object) this).Handle != IntPtr.Zero)
				return;

			IntPtr native_sampleSizes = JNIEnv.NewArray (sampleSizes);
			IntPtr native_sampleDurs = JNIEnv.NewArray (sampleDurs);
			try {
				JValue* __args = stackalloc JValue [8];
				__args [0] = new JValue (offset);
				__args [1] = new JValue (startTv);
				__args [2] = new JValue (sampleCount);
				__args [3] = new JValue (sampleSize);
				__args [4] = new JValue (native_sampleSizes);
				__args [5] = new JValue (sampleDur);
				__args [6] = new JValue (native_sampleDurs);
				__args [7] = new JValue (entry);
				...
				if (sampleSizes != null) {
					JNIEnv.CopyArray (native_sampleSizes, sampleSizes);
					JNIEnv.DeleteLocalRef (native_sampleSizes);
				}
				if (sampleDurs != null) {
					JNIEnv.CopyArray (native_sampleDurs, sampleDurs);
					JNIEnv.DeleteLocalRef (native_sampleDurs);
				}
			}
		}
