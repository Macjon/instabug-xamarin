...
namespace Org.Jcodec.Codecs.H264.Decode.Aso {
	...
	public partial class MBToSliceGroupMap : global::Java.Lang.Object {
		...
		public unsafe MBToSliceGroupMap (int[] groups, int[] indices, int[][] inverse)
			: base (IntPtr.Zero, JniHandleOwnership.DoNotTransfer)
		{
			if (((global::Java.Lang.Object) this).Handle != IntPtr.Zero)
				return;

			IntPtr native_groups = JNIEnv.NewArray (groups);
			IntPtr native_indices = JNIEnv.NewArray (indices);
			IntPtr native_inverse = JNIEnv.NewArray (inverse);
			try {
				JValue* __args = stackalloc JValue [3];
				__args [0] = new JValue (native_groups);
				__args [1] = new JValue (native_indices);
				__args [2] = new JValue (native_inverse);
				...
				if (groups != null) {
					JNIEnv.CopyArray (native_groups, groups);
				...
				if (inverse != null) {
					JNIEnv.CopyArray (native_inverse, inverse);
					JNIEnv.DeleteLocalRef (native_inverse);
				}
			}
		}
